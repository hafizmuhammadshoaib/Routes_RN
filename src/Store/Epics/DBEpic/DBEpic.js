...
export default class DBEpic {
    ...
    static setStopLocaiton(action$) {
        return action$.ofType(actionTypes.SET_STOP_LOCATION_PROG).switchMap(({ payload }) => {
            return service.put(`${BASE_URL}/userupdate/stoplocation`, { email: payload.email, lat: payload.lat, lng: payload.lng }, { 'authorization': payload.token, 'Content-Type': 'application/json' }).pluck("response").map(value => {
                return {
                    type: actionTypes.SET_STOP_LOCATION_SUCC,
                    payload: value
                }
            }).catch(err => {
                return Observable.of({ type: actionTypes.SET_STOP_LOCATION_FAIL, payload: err.response ? err.response.error : err.message })
            })
        })
    }
    static getAllBusInfo(action$) {
        return action$.ofType(actionTypes.GET_ALL_BUS_INFO_PROG).switchMap(({ payload }) => {
            return service.get(`${BASE_URL}/businfo/all`, { 'authorization': payload.token, 'Content-Type': 'application/json' }).pluck("response").map((value) => {
                return {
                    type: actionTypes.GET_ALL_BUS_INFO_SUCC,
                    payload: value.info
                }
            }).catch(err => {
                return Observable.of({ type: actionTypes.GET_ALL_BUS_INFO_FAIL, payload: err.response ? err.response.error : err.message })
            })
        })
    }
    static updateBusName(action$) {
        return action$.ofType(actionTypes.UPDATE_BUS_NAME_PROG).switchMap(({ payload }) => {
            return service.put(`${BASE_URL}/userupdate/busname`, { email: payload.email, busName: payload.busName }, { 'authorization': payload.token, 'Content-Type': 'application/json' }).pluck("response").map(value => {
                return {
                    type: actionTypes.UPDATE_BUS_NAME_SUCC,
                    payload: value
                }
            }).catch(err => {
                return Observable.of({ type: actionTypes.UPDATE_BUS_NAME_FAIL, payload: err.response ? err.response.error : err.message })
            })
        })
    }
